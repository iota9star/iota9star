...
The committed README uses **local `cards/*.svg` references** (markdown), not the HTML table — that's the `scripts/generate_masonry_local.py` variant. Those cards are gh-card.dev SVGs downloaded to `cards/`, so refresh them with `curl -sf https://gh-card.dev/repos/OWNER/REPO.svg -o cards/OWNER_REPO.svg` (filename = `/` → `_`). Verify freshness by grepping the SVG for `>N</text>` and comparing against the API star count — except mikan_flutter, whose card rounds stars to `>1.4k</text>`, so only its fork count (`>64</text>`) appears in the numeric grep. Live star counts as of 2026-09-14: mikan_flutter 1354, fjs 105, dpad 56, sakura-dmhy 37, kisssub 35, json_dart 24, void_signals 23, hora 19, f_limit 19, flexbox_layout 18, env2dart 11, svgo 9, resx 8, dotrix 8, dash_router 7, vcard_dart 3 (total 1736). iota9star user stats same day: 101 public repos, 282 followers, joined 2016-08-27.
